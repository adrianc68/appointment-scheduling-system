...
@Component({
  ...
  templateUrl: './register-appointment-as-client.component.html',
  styleUrl: './register-appointment-as-client.component.scss'
})
export class RegisterAppointmentAsClientComponent {
  ...
  constructor(private schedulerService: SchedulerService, private i18nService: I18nService, private clientService: ClientService, private errorUIService: ErrorUIService, private authenticationService: AuthenticationService) {
  ...
  async registerAppointmentAsClient(): Promise<void> {
    const accountData = await firstValueFrom(this.authenticationService.getAccountData());
    let clientUuid = accountData?.uuid;
