...
@Component({
  selector: 'app-edit-availability-time-slot',
  imports: [FormsModule, CommonModule, ...SHARED_STANDALONE_COMPONENTS],
  standalone: true,
  providers: [TaskStateManagerService],
  templateUrl: './edit-availability-time-slot.component.html',
  styleUrl: './edit-availability-time-slot.component.scss'
})
export class EditAvailabilityTimeSlotComponent {
  translationCodes = TranslationCodes;

  availabilitySlot: AvailabilityTimeSlot;
  ...
  constructor(private titleService: Title, private router: Router, private i18nService: I18nService, private loggingService: LoggingService, private schedulerService: SchedulerService, private stateManagerService: TaskStateManagerService) {
    this.availabilitySlot = this.router.getCurrentNavigation()?.extras.state?.["slot"];
  ...
  removeUnavailableTimeSlot(index: number) {
    this.availabilitySlot.unavailableTimeSlots.splice(index, 1);
  }
